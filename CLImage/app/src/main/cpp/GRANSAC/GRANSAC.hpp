   ...
   public:
    ...
    bool Estimate(const std::vector<std::shared_ptr<AbstractParameter>>& Data) {
        if (Data.size() <= t_NumParams) {
            std::cerr << "[ WARN ]: RANSAC - Number of data points is too less. Not doing anything."
                      << std::endl;
            return false;
        }
...
//#pragma omp parallel for
        for (int i = 0; i < m_MaxIterations; ++i) {
            ...
            std::shuffle(RemainderSamples.begin(), RemainderSamples.end(),
                         m_RandEngines[omp_get_thread_num()]);  // To avoid picking the same element
                                                                // more than once
            std::copy(RemainderSamples.begin(), RemainderSamples.begin() + t_NumParams,
                      RandomSamples.begin());
            // RemainderSamples.erase(RemainderSamples.begin(), RemainderSamples.begin() +
            // t_NumParams); // Remove the model data points from consideration. 2018: Turns out
